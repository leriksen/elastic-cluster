...
resource "azurerm_postgresql_flexible_server_active_directory_administrator" "ec_replica_aad" {
  depends_on = [
  ...
  principal_type      = "ServicePrincipal"
  resource_group_name = azurerm_resource_group.rg.name
  server_name         = local.ec_replica_name
  tenant_id           = data.azurerm_client_config.current.tenant_id
}
...
resource "azurerm_postgresql_flexible_server_configuration" "ec_replica_config" {
  for_each  = module.global.server_configs
  name      = each.key
  server_id = data.azurerm_postgresql_flexible_server.ec_replica.id
  value     = each.value
}
...
resource "azurerm_monitor_diagnostic_setting" "ec_replica" {
  name                       = "ds_ec"
  target_resource_id         = data.azurerm_postgresql_flexible_server.ec_replica.id
  log_analytics_workspace_id = azurerm_log_analytics_workspace.law.id

  dynamic "enabled_log" {
    for_each = data.azurerm_monitor_diagnostic_categories.ec_replica.log_category_groups
    content {
      category_group = enabled_log.value
    }
  }
